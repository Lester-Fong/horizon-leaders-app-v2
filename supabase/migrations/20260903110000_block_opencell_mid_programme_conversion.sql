drop function public.convert_visitor_to_member(uuid, uuid, uuid, text);
...
begin
  ...
  update public.visitors set status = 'converted', converted_member_id = new_member_id where id = p_visitor_id;
  return query select 'converted', new_member_id, null::uuid, null::text;
end;
$$;

comment on function public.convert_visitor_to_member(uuid, uuid, uuid, text) is 'Atomic Visitor conversion; unfinished OpenCell enrollment blocks mid-program conversion.';
revoke all on function public.convert_visitor_to_member(uuid, uuid, uuid, text) from public, anon, authenticated;
grant execute on function public.convert_visitor_to_member(uuid, uuid, uuid, text) to service_role;
